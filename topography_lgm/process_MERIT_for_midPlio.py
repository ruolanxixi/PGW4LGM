# Description: Process MERIT DEM data for mid-Pliocene (complement with GEBCO data for
...
if system == "Desktop":
    ...
    path_pmip4 = "/project/pr133/rxiang/data/pmip/setup/midPlio/"
    dir_work = "/scratch/snx3000/rxiang/Temp/dir_work/"
    dir_out = "/scratch/snx3000/rxiang/Temp/output/"
else:
...
ds = xr.open_dataset(path_pmip4 + "Plio_enh_topo_v1.0.nc")
topo_delta_pmip = ds["p4_topo"].values
lon_pmip = ds["lon"].values
lat_pmip = ds["lat"].values
ds.close()
ds = xr.open_dataset(path_pmip4 + "Modern_std_topo_v1.0.nc")
topo_delta_pmip -= ds["etopo1_topo"].values
ds.close()
...
print("lon_pmip shape: ",lon_pmip.shape)
print("lon_pmip dtype: ", lon_pmip.dtype)
print("topo_delta_pmip shape: ", topo_delta_pmip.shape)
print("topo_delta_pmip dtype: ", topo_delta_pmip.dtype)

lon_pmip = np.concatenate((np.array([-180.0], dtype=np.float32), lon_pmip, np.array([180.0], dtype=np.float32)))
len_lat = topo_delta_pmip.shape[0]
topo_delta_pmip_new_col = (topo_delta_pmip[:, 0] + topo_delta_pmip[:, -1]) / 2.
topo_delta_pmip = np.concatenate((topo_delta_pmip_new_col[:,np.newaxis], topo_delta_pmip, topo_delta_pmip_new_col[:, np.newaxis]),axis=1)
